...
import "./interface/ISimpleFarm.sol";

contract VerseStrategy is IStrategy, BaseUpgradeableStrategyUL {

  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  address public constant multiSigAddr = address(0xF49440C1F012d041802b25A73e5B0B9166a75c02);
  address public constant verseRouter = address(0xB4B0ea46Fe0E9e8EAB4aFb765b527739F2718671);
  ...
  function initializeBaseStrategy(
    ...
    address _lpt = ISimpleFarm(_rewardPool).stakeToken();
    require(_lpt == _underlying, "underlying mismatch");
    address _rwt = ISimpleFarm(_rewardPool).rewardToken();
    require(_rwt == _rewardToken, "reward token mismatch");
  ...
  function rewardPoolBalance() internal view returns (uint256 bal) {
      bal = ISimpleFarm(rewardPool()).balanceOf(address(this));
  }
  ...
  function exitRewardPool() internal {
    ...
    if (stakedBalance != 0) {
      ...
      uint256 earned = ISimpleFarm(_rewardPool).earned(address(this));
      if (earned > 0){
        ISimpleFarm(_rewardPool).exitFarm();
      } else {
        ISimpleFarm(_rewardPool).farmWithdraw(stakedBalance);
      }
    }
  }

  function partialWithdrawalRewardPool(uint256 amount) internal {
    ISimpleFarm(rewardPool()).farmWithdraw(amount);
  }

  function emergencyExitRewardPool() internal {
    uint256 stakedBalance = rewardPoolBalance();
    if (stakedBalance != 0) {
        ISimpleFarm(rewardPool()).farmWithdraw(stakedBalance); //don't claim rewards
    }
  }
  ...
  function enterRewardPool() internal {
    ...
    ISimpleFarm(_rewardPool).farmDeposit(entireBalance); //deposit and stake
  }
  ...
  */
  ...
  function _liquidateReward() internal {
    ...
    uint256 token0Amount = _swapToToken(_rewardToken, token0, toToken0);

    uint256 token1Amount = _swapToToken(_rewardToken, token1, toToken1);

    // provide token1 and token2 to Verse
    IERC20(token0).safeApprove(verseRouter, 0);
    IERC20(token0).safeApprove(verseRouter, token0Amount);

    IERC20(token1).safeApprove(verseRouter, 0);
    IERC20(token1).safeApprove(verseRouter, token1Amount);

    IUniswapV2Router02(verseRouter).addLiquidity(
      token0,
      token1,
      token0Amount,
      token1Amount,
  ...
  */
  function doHardWork() external onlyNotPausedInvesting restricted {
    ...
    uint256 earned = ISimpleFarm(_rewardPool).earned(address(this));
    if (earned > 0) {
      ISimpleFarm(_rewardPool).claimReward();
    }
    _liquidateReward();
    investAllUnderlying();
  }
