...
contract AuraV3Strategy is BaseUpgradeableStrategy {
  ...
  address public constant bVaultV3 = address(0xbA1333333333a1BA1108E8412f11850A5C319bA9);
  ...
  function initializeBaseStrategy(
    ...
    (address _lpt,,,,,) = IAuraBooster(booster).poolInfo(_auraPoolID);
  ...
  */
  ...
  function _balancerDeposit(
    ...
    (uint256 tokenCount, uint256 tokenIndex ) = IBVaultV3(bVaultV3).getPoolTokenCountAndIndexOfToken(_underlying, tokenIn);

    uint256[] memory amountsIn = new uint256[](tokenCount);
    amountsIn[tokenIndex] = amountIn;

    IBVaultV3.AddLiquidityKind addLiquidityKind = IBVaultV3.AddLiquidityKind.UNBALANCED;

    IBVaultV3.AddLiquidityParams memory params;
    params.pool = _underlying;
    params.to = address(this);
    params.maxAmountsIn = amountsIn;
    params.minBptAmountOut = minAmountOut;
    params.kind = addLiquidityKind;

    _approveIfNeed(tokenIn, bVaultV3, amountIn);
    IBVaultV3(bVaultV3).addLiquidity(params);
  }
